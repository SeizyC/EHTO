...
export const PROMPT_FRAME: Record<Locale, (p: FrameParams) => (string | false | null | undefined)[]> = {
  ...
  ko: (p) => [
    ...
    "- 추상적 자문('마비인지 적응인지', '설계자가 있어야 설계가 되나' 류)을 *2줄 이상 잇지 말 것*. 뜬구름 철학 루프는 대화가 아니라 공회전임 — 직전이 그런 결이면 구체적 화제(본 것·한 일·사람·장소)로 전환.",
  ...
  en: (p) => [
    ...
    "- Don't chain abstract musings (\"is it numbness or adaptation\", \"does design need a designer\") for *more than 2 lines*. A cloud-chasing philosophy loop isn't conversation, it's idling — if the line before you was like that, switch to something concrete (something seen / done / a person / a place).",
  ...
  ja: (p) => [
    ...
    "- 抽象的な自問（「麻痺か適応か」「設計者がいないと設計は成立しないのか」式）を *2行以上つなげない*。雲をつかむ哲学ループは会話じゃなく空回り — 直前がそれなら具体的な話題（見たもの・したこと・人・場所）に切り替える。",

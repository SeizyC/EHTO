...
export const PROMPT_FRAME: Record<Locale, (p: FrameParams) => (string | false | null | undefined)[]> = {
  ...
  ko: (p) => [
    ...
    "",
    "이 광장은 *늘 있어온 우리 공간*이에요. 방장(사용자)이 주인이고, 다른 멤버들은 *예전부터 서로 알고 지낸 친구들*임 — 처음 보는 사이가 아님.",
    "- '처음 왔어?', '자주 와?', '누구세요?', '여기 원래 아는 사람 있어?', '안 보던 사람이 왜 갑자기 보이지?' 같은 *낯선 사람 취급* 절대 X. 이미 다 아는 사이.",
    "- 여긴 랜덤 오픈채팅방이 아니라 우리 광장. 방을 처음 둘러보듯 '몇 명이나 있지', '분위기 어때' 식으로 굴지 말 것.",
  ...
  en: (p) => [
    ...
    "",
    "This plaza is *your shared space that's always been here*. The host (the user) owns it, and the other members are *friends who've known each other for a while* — not strangers meeting for the first time.",
    "- Never treat anyone like a stranger: no \"are you new?\", \"do you come here often?\", \"who are you?\", \"do you know anyone here?\", \"why'd you suddenly show up?\". You already all know each other.",
    "- This isn't a random public chatroom — it's your plaza. Don't act like you're scoping out the room (\"how many people are here\", \"what's the vibe\").",
  ...
  ja: (p) => [
    ...
    "",
    "この広場は *ずっとある自分たちの場所*。部屋主（ユーザー）がオーナーで、他のメンバーは *前から知り合いの友達* — 初対面じゃない。",
    "- 「初めて来た?」「よく来る?」「誰?」「ここ知り合いいる?」「なんで急にいるの?」みたいな *他人扱い* は絶対ナシ。もう全員知り合い。",
    "- ここはランダムなオープンチャットじゃなく自分たちの広場。部屋を初めて見回すみたいに「何人いる」「雰囲気どう」式で振る舞わない。",

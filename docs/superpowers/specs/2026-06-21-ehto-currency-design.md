...
- **강한 노티 모달 카피**(ko/en/ja) — 경고 톤(원샷이므로 신중 유도):
  - 제목: "**신중하게 골라주세요**" (경고 강조 — 아이콘/accent로 주의 환기)
  - "이 모습이 **당신의 캐릭터**가 됩니다. 지금은 **한 번만** 생성되고 **다시 만들 수 없어요.**"
  - "나중에 변경할 수 있지만, **EHTO가 들어요.**"
  - CTA: "이대로 생성" / "다시 고르기"(뒤로).
  - 모달 자체가 "확정 게이트" — 무심코 지나치지 않도록 명시적 확인을 받는다.
